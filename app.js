...
/* ============================================================
   HABILLEMENT V3 — Suivi individuel des renouvellements
   ============================================================ */
...
function habBaseId(id){ return id.replace(/_\d+$/,""); }
function habDernier(mat,id){ const h=fcHabillement.find(h=>h.agent_matricule===mat&&h.effet_id===id); return h?h.date_renouvellement:null; }
function habStatut(mat,id){
  const ref=HAB_REF.find(r=>r.id===habBaseId(id));
  if(!ref||!ref.periode) return "manuel";
  const d=habDernier(mat,id); if(!d) return "renouvelable";
  const j=Math.floor((Date.now()-new Date(d))/86400000),r=ref.periode-j;
  if(r<=0) return "renouvelable"; if(r<=HAB_SEUIL) return "bientot"; return "ok";
}
function habProchain(mat,id){
  const ref=HAB_REF.find(r=>r.id===habBaseId(id)); if(!ref||!ref.periode) return null;
  const d=habDernier(mat,id); if(!d) return null;
  const dt=new Date(d); dt.setDate(dt.getDate()+ref.periode); return dt;
}
function habFmt(s){ if(!s) return null; return new Date(s).toLocaleDateString("fr-FR"); }
function habPeriodeLabel(j){ if(!j) return null; if(j<=183) return "2× / an"; if(j<=365) return "1× / an"; if(j<=548) return "1× / 18 mois"; if(j<=730) return "1× / 2 ans"; return "1× / 3 ans"; }
function habEffetsPour(agent){
  ...
  HAB_REF.filter(r=>isSPP?r.spp:r.spv).forEach(ref=>{
    ...
    if(nb<=1){ rows.push({id:ref.id,label:ref.label,ref,slotNum:0}); }
    else { for(let i=1;i<=nb;i++) rows.push({id:`${ref.id}_${i}`,label:`${ref.label} #${i}`,ref,slotNum:i}); }
  });
  return rows;
}

function habGetFType(){ return ((document.getElementById("habFilterType")||{}).value||"").toUpperCase(); }
function habAgents(){
  const fType=habGetFType();
  return (typeof personnelList!=="undefined"?personnelList:[]).filter(p=>{
...
function habComputeGlobal(){
  ...
  habAgents().forEach(a=>{ habEffetsPour(a).forEach(ef=>{ const s=habStatut(a.matricule,ef.id); if(s==="renouvelable")cR++; else if(s==="bientot")cS++; else if(s==="ok")cOk++; else cM++; }); });
  return {cR,cS,cOk,cM};
}
...
function renderHabGlobalStats(){
  ...
  if($id("habGcountAgents")){
    const nb=habAgents().length; $id("habGcountAgents").textContent=nb;
    const sp=$id("habGcountAgents").nextElementSibling;
    if(sp) sp.textContent=`Agent${nb>1?"s":""}${fType?" "+fType:""}`;
  }
}
...
function renderHabAgentListPanel(mode){
  ...
  const concerned=habAgents().filter(a=>habEffetsPour(a).some(ef=>ef.ref.periode&&habStatut(a.matricule,ef.id)===mode)).sort((a,b)=>(a.nom||"").localeCompare(b.nom||"","fr"));
  ...
  items.innerHTML=concerned.map(a=>{
    ...
    const col=mode==="renouvelable"?"#e53935":"#fb8c00";
  ...
  items.querySelectorAll("[data-hab-chip-mat]").forEach(chip=>{
    chip.onmousedown=e=>{ e.preventDefault(); const a=habAgents().find(x=>x.matricule===chip.dataset.habChipMat); if(a) habSelectAgent(a); };
  });
}
...
function renderHabAgentStats(agent){
  ...
  let cR=0,cS=0,cOk=0,cM=0;
  effets.forEach(ef=>{ const s=habStatut(agent.matricule,ef.id); if(s==="renouvelable")cR++; else if(s==="bientot")cS++; else if(s==="ok")cOk++; else cM++; });
  const $id=id=>document.getElementById(id);
...
function renderHabTable(){
  ...
  if(!agent){
    thead.innerHTML=`<tr><th>Catégorie</th><th>Effet</th><th>SPP</th><th>SPV</th><th>Périodicité</th><th>Durée (jours)</th></tr>`;
  ...
  const effets=habEffetsPour(agent);
  renderHabAgentStats(agent);
  thead.innerHTML=`<tr><th>Catégorie</th><th>Effet</th><th>Périodicité</th><th>Dernier renouvellement</th><th>Prochain prévu</th><th>Statut</th><th>Action</th></tr>`;
  ...
  tbody.innerHTML=cats.map(cat=>{
    ...
    return `<tr class="hab-row-group"><td colspan="7">${cat}</td></tr>`+efs.map(ef=>{
      ...
      const prochainStr=!periode?`<span style="color:#bbb">—</span>`:!derniere?`<span style="color:#fb8c00;font-size:.8rem;font-style:italic">Non renseigné</span>`:`<span${isLate?' style="color:#e53935;font-weight:600"':''}>${prochain.toLocaleDateString("fr-FR")}</span>`;
  ...
  tbody.querySelectorAll("[data-hab-date-mat]").forEach(input=>{
    input.onchange=async()=>{
      const mat=input.dataset.habDateMat,ef=input.dataset.habDateEf,val=input.value;
      if(!val) return;
      ...
      if(row){
        const s=habStatut(mat,ef),prochain=habProchain(mat,ef),cells=row.querySelectorAll("td");
        if(cells[4]){ const isLate=prochain&&prochain<new Date(); cells[4].innerHTML=!prochain?`<span style="color:#bbb">—</span>`:`<span${isLate?' style="color:#e53935;font-weight:600"':''}>${prochain.toLocaleDateString("fr-FR")}</span>`; }
        const dotCls=s==="renouvelable"?"hab-dot-red":s==="bientot"?"hab-dot-orange":s==="ok"?"hab-dot-green":"hab-dot-grey";
        const chipCls=s==="renouvelable"?"hab-chip-red":s==="bientot"?"hab-chip-orange":s==="ok"?"hab-chip-green":"hab-chip-grey";
        const slabel=s==="renouvelable"?"Renouvelable":s==="bientot"?"Bientôt":s==="ok"?"À jour":"Manuel";
        if(cells[5]) cells[5].innerHTML=`<span class="hab-chip ${chipCls}"><span class="hab-dot ${dotCls}"></span>${slabel}</span>`;
        const btn=cells[6]&&cells[6].querySelector(".hab-reg-btn"); if(btn) btn.disabled=s==="ok";
      }
      renderHabGlobalStats();
    };
  });
  ...
  tbody.querySelectorAll("[data-hab-today-mat]").forEach(btn=>{
    btn.onclick=async()=>{
      ...
      await habSaveDate(btn.dataset.habTodayMat,btn.dataset.habTodayEf,new Date().toISOString().split("T")[0]);
      if(typeof toast==="function") toast("Renouvellement enregistré ✓");
      renderHabTable(); renderHabGlobalStats();
    };
  });
}
...
function renderHabPicker(q){
  const fType=habGetFType();
  const wrap=document.getElementById("habPickerWrap"),list=document.getElementById("habPickerList");
  if(!wrap||!list) return;
...
function initHabHelp(){
  ...
  if(!btn||btn._hh) return; btn._hh=true;
  if(!document.getElementById("habHelpDialog")){
    const dlg=document.createElement("dialog"); dlg.id="habHelpDialog";
    dlg.innerHTML=`<div class="hab-help-hd"><h3>📋 Habillement — Aide &amp; Annexe VI</h3><button onclick="document.getElementById('habHelpDialog').close()">✕</button></div>
    <div class="hab-help-bd">
      <h4>Comment utiliser cet onglet ?</h4>
      <p>Par défaut, vous voyez le référentiel général (Annexe VI). Tapez un nom ou matricule pour sélectionner un agent — le tableau bascule sur ses données avec dernier renouvellement et prochain prévu.</p>
      <p>Pour modifier une date : cliquez sur le champ date de la ligne. Pour enregistrer aujourd'hui : bouton "Aujourd'hui".</p>
      <p>Les badges globaux comptent les effets sur tous les agents. Cliquez "Renouvelables" ou "Bientôt" pour voir la liste des agents concernés.</p>
      <h4>Légende</h4>
      <div class="hab-help-legend">
        ...
        <div><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#fb8c00;margin-right:4px"></span><strong>Bientôt</strong> — moins de 30 jours</div>
        <div><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#43a047;margin-right:4px"></span><strong>À jour</strong></div>
        <div><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#bdbdbd;margin-right:4px"></span><strong>Manuel</strong> — décision C.RH</div>
      </div>
      <h4>Référentiel Annexe VI</h4>
      <table class="hab-help-tbl"><thead><tr><th>Effet</th><th>SPP</th><th>SPV</th><th>Périodicité</th></tr></thead><tbody>
        ...
        <tr><td>Pantalon TSI</td><td>4 / 18 mois</td><td>3 / 18 mois</td><td>~1 tous les 18 mois</td></tr>
        <tr><td>Polo manches courtes</td><td>✓</td><td>✓</td><td>2 par an</td></tr>
        ...
        <tr><td>Socquettes chaussures basses</td><td>✓</td><td>✓</td><td>3 / 18 mois</td></tr>
        <tr><td>Calot / Chaussures basses</td><td>✓</td><td>✓</td><td>Manuel C.RH</td></tr>
        <tr><td class="hab-help-cat" colspan="4">Sport — SPP uniquement</td></tr>
        <tr><td>Sweat / Pantalon / Collant / Cuissard / Maillot / Short</td><td>✓</td><td>—</td><td>1 tous les 3 ans</td></tr>
        <tr><td>Chaussettes de sport</td><td>✓</td><td>—</td><td>2 paires / an</td></tr>
        <tr><td>Chaussures de sport</td><td>✓</td><td>—</td><td>2 paires / 2 ans</td></tr>
      </tbody></table>
      <h4>Règle générale (Art. 18)</h4>
      <p>"Renouvelable" = éligible, pas obligatoire. Un effet en bon état n'est pas remplacé même si le délai est dépassé.</p>
    </div>`;
...
function initHabillement(){
  ...
  if(search&&!search._hInit){
    ...
    document.addEventListener("click",e=>{ if(!e.target.closest("#habPickerWrap")&&!e.target.closest("#habAgentSearch")){ const w=document.getElementById("habPickerWrap"); if(w) w.style.display="none"; } });
  }
  const fType=document.getElementById("habFilterType");
  if(fType&&!fType._hInit){
    fType._hInit=true;
    fType.onchange=()=>{ renderHabGlobalStats(); if(habGlobalFilter) renderHabAgentListPanel(habGlobalFilter); const q=(document.getElementById("habAgentSearch")||{}).value||""; if(q) renderHabPicker(q); };
  }
  const clearBtn=document.getElementById("habClearAgent");
  if(clearBtn&&!clearBtn._hInit){ clearBtn._hInit=true; clearBtn.onclick=habClearAgent; }
  const closePanel=document.getElementById("habCloseAgentList");
  if(closePanel&&!closePanel._hInit){
    closePanel._hInit=true;
    closePanel.onclick=()=>{ habGlobalFilter=""; ["habGstatRenew","habGstatSoon"].forEach(id=>document.getElementById(id)?.classList.remove("hab-gstat-active")); renderHabAgentListPanel(""); };
  }
  const bindGstat=(btnId,mode)=>{ const btn=document.getElementById(btnId); if(btn&&!btn._hInit){ btn._hInit=true; btn.onclick=()=>{ habGlobalFilter=habGlobalFilter===mode?"":mode; ["habGstatRenew","habGstatSoon"].forEach(id=>document.getElementById(id)?.classList.remove("hab-gstat-active")); if(habGlobalFilter) btn.classList.add("hab-gstat-active"); renderHabAgentListPanel(habGlobalFilter); }; } };
  bindGstat("habGstatRenew","renouvelable"); bindGstat("habGstatSoon","bientot");
  initHabHelp();
  renderHabGlobalStats();
  renderHabTable();
}

document.querySelectorAll(".tech-nav").forEach(btn=>{ if(btn.dataset.tab==="habillement") btn.addEventListener("click",()=>setTimeout(initHabillement,0)); });

(function habWait(){ if(typeof supabase!=="undefined"&&supabase&&typeof supabase.from==="function") loadHabillement(); else setTimeout(habWait,200); })();

/* ============================================================
   FICHE VÉHICULE V33 — Remplace Médias de l'inventaire
   ============================================================ */
...
async function uploadCoverPhoto(vehicleId,file){
  let dataUrl;
  try{ dataUrl=typeof compressImageV32==="function"?await compressImageV32(file,1400,0.82):await new Promise((res,rej)=>{ const r=new FileReader(); r.onload=()=>res(r.result); r.onerror=rej; r.readAsDataURL(file); }); }
  catch(e){ throw new Error("Lecture image impossible"); }
  const path=`covers/${vehicleId}_${Date.now()}.jpg`;
  try{
    const blob=await(await fetch(dataUrl)).blob();
    const {data,error}=await supabase.storage.from("firecheck-photos").upload(path,blob,{contentType:"image/jpeg",upsert:true});
    ...
    console.warn("Storage upload échoué, fallback local:",e);
...
function renderFicheVehicule(root,vehicle){
  ["#mediaManagerV30","#mediaManagerV31",".fiche-vehicule-card"].forEach(sel=>root.querySelectorAll(sel).forEach(el=>el.remove()));
  ...
  saveBtn.onclick=async()=>{
    ...
    await saveVehicleInfo(vehicle.id,{
    ...
    saveBtn.disabled=false; saveBtn.textContent="💾 Enregistrer la fiche";
    savedBadge.style.display="inline"; setTimeout(()=>savedBadge.style.display="none",2500);
...
getCoverPhoto=function(vehicleId){ const info=fcVehicleInfo[vehicleId]; if(info&&info.cover_url) return info.cover_url; return getCoverPhotoOrigV33(vehicleId); };
...
window.print=function(){
  ...
  if(printArea){
    ...
    printArea.querySelector(".print-cover-fiche")?.remove();
...
(function waitVI(){ if(typeof supabase!=="undefined"&&supabase&&typeof supabase.from==="function") loadVehicleInfo(); else setTimeout(waitVI,200); })();
